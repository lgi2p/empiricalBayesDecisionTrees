...
path <- "C:..."
setwd(path)
library(rpart); library(rpart.plot); library(caret); library(WriteXLS); 
library(ggpubr); library(MASS); source("myLib.R")

# INPUTS:
# available datasets: banana, bankLoan, banknote, mammo, pima, ticTacToe, wisconsin

datasetNames  <- c("banana", "banknote", "mammo", "pima", "ticTacToe", "wisconsin")
cps           <- c(0, 0.001, 0.005, 0.01, 0.05)
nRuns         <- 20
nFold         <- 10
granUncEval   <- 10 # granularity of the search in the uncertain predictive probability interval [p1Inf, p1Sup], i.e. number of considered points
uncertainEval <- T
...
saveXLS(finalResults)

boxplots <- generate_boxplots(inputs, finalResults)
